...
/**
 * fadeIn() — затемняет экран (fade to black).
 * fadeOut() — убирает затемнение.
 */
public class FadeInFunction implements ScriptFunction {
    ...
    public Object execute(List<Object> args, CommandSourceStack source, ScriptContext context) {
        ...
        props.put("fadeout", false);

        for (Object arg : args) {
            if (arg instanceof Player) continue;
            if (arg instanceof Number n) {
                props.put("time", n.doubleValue());
            } else if (arg instanceof String s) {
                if (!props.containsKey("text")) {
                    props.put("text", s);
                } else if (!props.containsKey("subtitle")) {
                    props.put("subtitle", s);
                }
            }
        }
    ...
    private static ServerPlayer resolvePlayer(List<Object> args, CommandSourceStack source) {
        if (!args.isEmpty()) {
            ...
            if (first instanceof Player p && p instanceof ServerPlayer sp) return sp;
