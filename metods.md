...
| `addMobDrop(mob_id, item_id, [min=1], [max=1], [chance=100], [replace=false])` | Добавляет кастомный предмет в лут ванильного или модового моба (например: `"minecraft:zombie"`, `"minecraft:diamond"`, `1`, `1`, `50`). Аргумент `replace` отменяет выпадение оригинального дропа, оставляя только кастомный. |
| `addBlockDrop(block_id, item_id, [min=1], [max=1], [chance=100], [replace=false])` | Настраивает кастомный дроп при ломании блока игроком. Если `replace` = `true`, блок будет сломан без оригинального дропа (только с кастомным предметом). |
...
| `drop_item` | Строка, ID предмета, который выпадает при смерти NPC (например, `"minecraft:iron_sword"`) |
